# This code was modified from e3nn-jax (https://github.com/e3nn/e3nn-jax)
#
...
def scatter_sum(
    ...
    *,
    ...
    Returns:
        `jax.numpy.ndarray` of shape ``(output_size, ...)``
    """
    ...
    *,
    ...
    if nel is not None:
        if output_size is not None:
            raise ValueError(
                "output_size must not be specified if nel is specified")
        output_size = nel.shape[0]
        num_elements = data.shape[0]
        dst = jnp.repeat(jnp.arange(output_size), nel,
                         total_repeat_length=num_elements)
    ...
    if not (dst.shape == data.shape[: dst.ndim]):
        raise ValueError(
            (
                f"trying to do scatter_{op} with dst.shape={dst.shape} and data.shape={data.shape}"
    ...
    if output_size is not None and map_back is True:
        raise ValueError(
            "output_size must not be specified if map_back is True")
    ...
    def _op(x):
        ...
        if op == "sum":
            ...
            return z.at[(dst,)].min(x, indices_are_sorted=indices_are_sorted, mode=mode)
        elif op == "max":
