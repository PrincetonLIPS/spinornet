...
class ScalarAffineTest(parameterized.TestCase):
    ...
    def test_shapes_are_correct(self):
        ...
        bij_with_log_scale = scalar_affine.ScalarAffine(
            shift, log_scale=log_scale)
    ...
    def test_batched_parameters(self, scale_batch_shape, shift_batch_shape,
        ...
        for i in range(np.prod(output_batch_shape)):
            bijector = scalar_affine.ScalarAffine(
                shift[i], jnp.exp(log_scale[i]))
            ...
            np.testing.assert_allclose(
                this_logdet_fwd, logdet_fwd[i], atol=1e-4)
            np.testing.assert_allclose(
                this_logdet_inv, logdet_inv[i], atol=1e-4)
