...
import chex
from spinornet.distrax_alpha._src.bijectors import bijector as distrax_bijector
...
class MaskedCouplingTest(parameterized.TestCase):
    ...
    def test_batched_mask(self, mask_batch_shape, input_batch_shape):
        ...
        x = jnp.broadcast_to(x, output_batch_shape +
                             (5, 6)).reshape((-1, 5, 6))
        y = y.reshape((-1, 5, 6))
        z = z.reshape((-1, 5, 6))
        ...
        for i in range(np.prod(output_batch_shape)):
            ...
            np.testing.assert_allclose(
                this_logdet_fwd, logdet_fwd[i], atol=1e-5)
            np.testing.assert_allclose(
                this_logdet_inv, logdet_inv[i], atol=1e-5)
