...
from spinornet.distrax_alpha._src.bijectors import bijector as base
...
from spinornet.distrax_alpha._src.bijectors import triangular_linear
from spinornet.distrax_alpha._src.bijectors import unconstrained_affine
...
class LowerUpperTriangularAffine(chain.Chain):
    ...
    * The upper-triangular part of M (including the diagonal) becomes U.
    ...
    def __init__(self, matrix: Array, bias: Array):
        ...
        self._upper = triangular_linear.TriangularLinear(
            matrix, is_lower=False)
        dim = matrix.shape[-1]
        # Replace diagonal with ones.
        lower = jnp.eye(dim) + jnp.tril(matrix, -1)
