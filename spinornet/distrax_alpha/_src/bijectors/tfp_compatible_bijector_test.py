...
from spinornet.distrax_alpha._src.bijectors.tfp_compatible_bijector import tfp_compatible_bijector
import jax
...
class TFPCompatibleBijectorTest(parameterized.TestCase):
    ...
    def test_transformed_distribution(
        ...
        dist_with_wrapped = tfd.TransformedDistribution(
            base_dist, wrapped_bijector)
        ...
        with self.subTest('log_prob'):
            ...
            np.testing.assert_allclose(
                log_prob_wrapped, log_prob_tfp_only, rtol=RTOL)
    ...
    @parameterized.named_parameters(
        ...
        ('tanh', Tanh, tfb.Tanh, np.array(
            [-0.1, 0.01, 0.1], dtype=np.float32)),
    ...
    def test_log_det_jacobian(self, dx_bijector_fn, tfp_bijector_fn, event):
        ...
        with self.subTest('experimental_compute_density_correction'):
            ...
            self.assertIsInstance(
                space, tfp.experimental.tangent_spaces.FullSpace)
