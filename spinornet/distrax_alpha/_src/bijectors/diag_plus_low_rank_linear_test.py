...
import chex
from spinornet.distrax_alpha._src.bijectors.diag_plus_low_rank_linear import DiagPlusLowRankLinear
...
class DiagPlusLowRankLinearTest(parameterized.TestCase):
    ...
    def test_properties(self):
        ...
        np.testing.assert_allclose(
            bij.u_matrix, np.full((4, 2), 2.), atol=1e-6)
        np.testing.assert_allclose(
            bij.v_matrix, np.full((4, 2), 3.), atol=1e-6)
        np.testing.assert_allclose(
    ...
    def test_batched_parameters(self, diag_batch_shape, u_matrix_batch_shape,
        ...
        u_matrix = jax.random.uniform(
            next(prng), u_matrix_batch_shape + (4, 1))
        v_matrix = jax.random.uniform(
            next(prng), v_matrix_batch_shape + (4, 1))
        ...
        diag = jnp.broadcast_to(
            diag, output_batch_shape + (4,)).reshape((-1, 4))
        ...
        for i in range(np.prod(output_batch_shape)):
            ...
            this_y, this_logdet_fwd = self.variant(
                bij.forward_and_log_det)(x[i])
            this_z, this_logdet_inv = self.variant(
                bij.inverse_and_log_det)(x[i])
            np.testing.assert_allclose(this_y, y[i], atol=1e-6)
            np.testing.assert_allclose(this_z, z[i], atol=1e-6)
            np.testing.assert_allclose(
                this_logdet_fwd, logdet_fwd[i], atol=1e-6)
            np.testing.assert_allclose(
                this_logdet_inv, logdet_inv[i], atol=1e-6)
