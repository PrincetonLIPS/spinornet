...
from spinornet.distrax_alpha._src.utils import jittable
import jax
...
class HMM(jittable.Jittable):
    ...
    def forward(self,
        ...
        # post-process
        alpha_hist = jnp.vstack(
            [alpha_0.reshape(1, self._n_states), alpha_hist])
        (_, log_ll) = carry
    ...
    def viterbi(self, obs_seq: chex.Array) -> chex.Array:
        ...
        def viterbi_backward(state, most_likely_sources):
            ...
            most_likely = jnp.sum(most_likely_sources *
                                  state).astype(jnp.int64)
