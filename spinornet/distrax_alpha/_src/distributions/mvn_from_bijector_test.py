...
import chex
from spinornet.distrax_alpha._src.bijectors import bijector
from spinornet.distrax_alpha._src.bijectors.diag_linear import DiagLinear
from spinornet.distrax_alpha._src.bijectors.triangular_linear import TriangularLinear
from spinornet.distrax_alpha._src.distributions.mvn_from_bijector import MultivariateNormalFromBijector
...
class MultivariateNormalFromBijectorTest(parameterized.TestCase):
    ...
    def test_raises_on_wrong_inputs(
        ...
        bij = DummyBijector(event_ndims_in, event_ndims_out,
    ...
    def test_sample(self):
        ...
        dist = MultivariateNormalFromBijector(loc, scale, batch_shape=())
        num_samples = 100_000
        def sample_fn(seed): return dist.sample(
            seed=seed, sample_shape=num_samples)
        ...
        for mode in ['distrax_to_distrax', 'distrax_to_tfp', 'tfp_to_distrax']:
            with self.subTest(mode=mode):
                if mode == 'distrax_to_distrax':
                    result1 = self.variant(
                        dist1_distrax.kl_divergence)(dist2_distrax)
                    result2 = self.variant(
                        dist2_distrax.kl_divergence)(dist1_distrax)
                elif mode == 'distrax_to_tfp':
                    result1 = self.variant(
                        dist1_distrax.kl_divergence)(dist2_tfp)
                    result2 = self.variant(
                        dist2_distrax.kl_divergence)(dist1_tfp)
                elif mode == 'tfp_to_distrax':
                    result1 = self.variant(
                        dist1_tfp.kl_divergence)(dist2_distrax)
                    result2 = self.variant(
                        dist2_tfp.kl_divergence)(dist1_distrax)
                np.testing.assert_allclose(
                    result1, expected_result1, rtol=1e-3)
                np.testing.assert_allclose(
                    result2, expected_result2, rtol=1e-3)
    ...
    def test_kl_divergence_non_diag_distributions(self):
        ...
        dist1_tfp = tfd.MultivariateNormalTriL(
            loc=loc1, scale_tril=scale_tril1)
        ...
        dist2_tfp = tfd.MultivariateNormalTriL(
            loc=loc2, scale_tril=scale_tril2)
