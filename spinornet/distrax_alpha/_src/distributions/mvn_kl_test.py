                ...
                with self.subTest(method=method, mode=mode):
                    if mode == 'distrax_to_distrax':
                        ...
                        result1 = self.variant(
                            getattr(dist1_distrax, method))(dist2_tfp)
                        result2 = self.variant(
                            getattr(dist2_distrax, method))(dist1_tfp)
                    elif mode == 'tfp_to_distrax':
                        result1 = self.variant(
                            getattr(dist1_tfp, method))(dist2_distrax)
                        result2 = self.variant(
                            getattr(dist2_tfp, method))(dist1_distrax)
                    np.testing.assert_allclose(
                        result1, expected_result1, rtol=0.02)
                    np.testing.assert_allclose(
                        result2, expected_result2, rtol=0.02)
