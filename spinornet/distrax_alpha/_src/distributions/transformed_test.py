...
import chex
from spinornet.distrax_alpha._src.bijectors import block
from spinornet.distrax_alpha._src.bijectors import lambda_bijector
from spinornet.distrax_alpha._src.bijectors import masked_coupling
from spinornet.distrax_alpha._src.bijectors import scalar_affine
from spinornet.distrax_alpha._src.bijectors import sigmoid
from spinornet.distrax_alpha._src.distributions import normal
from spinornet.distrax_alpha._src.distributions import transformed
from spinornet.distrax_alpha._src.utils import conversion
import haiku as hk
import jax
...
class TransformedTest(parameterized.TestCase):
    ...
    def test_event_shape(self, mu, sigma, base_dist):
        ...
        tfp_dist = tfd.TransformedDistribution(
            conversion.to_tfp(base), bijector)
    ...
    def test_sample_shape(self, mu, sigma, sample_shape, base_dist):
        ...
        tfp_samples = tfp_dist.sample(
            sample_shape=sample_shape, seed=self.seed)
    ...
    def test_bijector_that_assumes_batch_dimensions(self):
        ...
        def forward(x):
            network = hk.Sequential(
                [hk.Flatten(preserve_dims=1), hk.Linear(3)])
            return network(x)
    ...
    def test_kl_divergence_raises_on_event_shape(self):
        base_dist1 = tfd.MultivariateNormalDiag(
            [0.1, 0.5, 0.9], [0.1, 1.1, 2.5])
        base_dist2 = tfd.Normal(-0.1, 1.5)
