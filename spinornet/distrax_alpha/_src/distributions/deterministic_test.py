...
import chex
from spinornet.distrax_alpha._src.distributions import deterministic
...
class DeterministicTest(equivalence.EquivalenceTest, parameterized.TestCase):
    ...
    def test_atol(self, atol):
        ...
        dist = self.distrax_cls(**dist_params)
        broadcasted_atol = np.zeros(
            (3,)) if atol is None else atol * np.ones((3,))
    ...
    def test_rtol(self, rtol):
        ...
        dist = self.distrax_cls(**dist_params)
        broadcasted_rtol = np.zeros(
            (3,)) if rtol is None else rtol * np.ones((3,))
