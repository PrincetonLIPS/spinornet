...
from spinornet.distrax_alpha._src.distributions.tfp_compatible_distribution import tfp_compatible_distribution
...
class TFPCompatibleDistributionNormal(parameterized.TestCase):
    ...
    def setUp(self):
        ...
        self.assertion_fn = lambda x, y: np.testing.assert_allclose(
            x, y, rtol=RTOL)
        self.base_dist = Normal(loc=jnp.array(
            [0., 0.]), scale=jnp.array([1., 1.]))
    ...
    def test_event_shape(self):
        self.assertEqual(self.wrapped_dist.event_shape,
    ...
    def test_batch_shape(self):
        self.assertEqual(self.wrapped_dist.batch_shape,
...
class TFPCompatibleDistributionCategorical(TFPCompatibleDistributionNormal):
    ...
    def setUp(self):
        ...
        self.distrax_second_dist = Categorical(
            probs=jnp.array([0.2, 0.2, 0.6]))
        self.tfp_second_dist = tfd.Categorical(
            probs=jnp.array([0.2, 0.2, 0.6]))
