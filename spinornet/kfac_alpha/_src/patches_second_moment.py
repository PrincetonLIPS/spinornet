...
class _ConvSpec:
    ...
    def expand_spatial_axes(self) -> "_ConvSpec":
        ...
        n_axis = self.n_axis + \
            sum(self.n_axis > axis for axis in self.spatial_axes)
        c_axis = self.c_axis + \
            sum(self.c_axis > axis for axis in self.spatial_axes)
        spatial_axes = []
        for axis in self.spatial_axes:
            spatial_axes.append(
                axis + sum(axis > a for a in self.spatial_axes))
    ...
    def change_nhwc_to_ihwo(self) -> "_ConvSpec":
        ...
        order = [i - 2 if i > self.spatial_axes[1]
                 else i for i in self.order[:4]]
...
def _data_format_to_dim_numbers(
    ...
    if not isinstance(data_format, str):
        raise ValueError(
            "data_format must be either a python string or `None`.")
...
def _num_conv_locations_full_spec(
    ...
    for in_dim, k_dim, stride, padding in zip(
        ...
        num_locations *= _ceil(in_dim +
                               padding[0] + padding[1] - k_dim + 1, stride)
    ...
    def general_loop_body(i, image):
        ...
        if weighting_array is not None:
            if weighting_array.shape[in_spec.c_axis] == 1:
                ...
                wf_spatial = [weighting_array.shape[a]
                              for a in in_spec.spatial_axes]
                wf_sizes = in_spec.create_shape(
                    wf_n, jnp.ones([]), *wf_spatial)
...
def patches_moments(
    ...
    if inputs_dilation not in (None, 1, (1, 1)):
        raise ValueError(
            "`patches_second_moment` does not support input dilation.")
