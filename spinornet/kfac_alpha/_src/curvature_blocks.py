...
from spinornet.kfac_alpha._src import layers_and_loss_tags as tags
from spinornet.kfac_alpha._src import patches_second_moment as psm
from spinornet.kfac_alpha._src import tag_graph_matcher as tgm
...
class CurvatureBlock(utils.Finalizable):
    ...
    def dtype(self) -> DType:
        ...
        dtypes = set(p.aval.dtype for p in self.parameter_variables)
        if len(dtypes) > 1:
...
class Diagonal(CurvatureBlock, abc.ABC):
    ...
    def _multiply_matpower_unscaled(
        ...
        factors = tuple(
            f.value + identity_weight for f in state.diagonal_factors)
...
class Full(CurvatureBlock, abc.ABC):
    ...
    def single_vector_to_parameters_list(
        ...
        for shape in self.parameters_shapes:
            ...
            parameters_shaped_list.append(
                vector[index: index + size].reshape(shape))
            index += size
    ...
    def _multiply_matpower_unscaled(
        ...
        if power == 1:
            ...
            if power == -1:
                ...
                result = jnp.matmul(
                    jnp.linalg.matrix_power(matrix, power), vector)
            ...
            for power in exact_powers:
                ...
                if power == -1:
                    ...
                    matrix = state.matrix.value + \
                        identity_weight * jnp.eye(self.dim)
    ...
    def _to_dense_unscaled(self, state: "Full.State") -> Array:
        ...
        return utils.block_permuted(
            state.matrix.value,
            block_sizes=[utils.product(shape)
                         for shape in self.parameters_shapes],
...
class KroneckerFactored(CurvatureBlock, abc.ABC):
    ...
    def _init(
        ...
        for i, d in enumerate(self.grouped_array_shape):
            ...
            if cache_eigenvalues or exact_powers_to_cache:
                cache[f"{i}_factor_eigenvalues"] = jnp.zeros(
                    (d,), dtype=self.dtype)

            if exact_powers_to_cache:
                cache[f"{i}_factor_eigen_vectors"] = jnp.zeros(
    ...
    def _multiply_matpower_unscaled(
        ...
        if power == 1:
            ...
            result = utils.kronecker_eigen_basis_axis_mul_v(
                q, eigenvalues, vector)
...
class NaiveFull(Full):
    ...
    def _update_curvature_matrix_estimate(
        ...
        params_grads = jax.tree_util.tree_leaves(
            estimation_data["params_tangent"])
        params_grads = jax.tree_map(lambda x: x.flatten(), params_grads)
        grads = jnp.concatenate(params_grads, axis=0)

        state.matrix.update(jnp.outer(grads, grads) /
                            batch_size, ema_old, ema_new)
...
class DenseFull(Full):
    ...
    def _update_curvature_matrix_estimate(
        ...
        if self.number_of_parameters == 2:
            params_tangents = jnp.concatenate(
                [params_tangents, dy[:, None]], axis=1)
        params_tangents = jnp.reshape(params_tangents, [batch_size, -1])

        matrix_update = jnp.matmul(
            params_tangents.T, params_tangents) / batch_size
...
class Conv2DDiagonal(Diagonal):
    ...
    def _update_curvature_matrix_estimate(
        ...
        if self.has_bias:
            sum_axis = tuple(
                range(1, dy.ndim - len(self.parameters_shapes[1])))
...
class Conv2DTwoKroneckerFactored(TwoKroneckerFactored):
    ...
    def compute_outputs_stats(self, tangent_of_output: Array) -> Array:
        ...
        lhs_str = utils.replace_char(
            _ALPHABET[:4], "y", self.outputs_channel_index)
        rhs_str = utils.replace_char(
            _ALPHABET[:4], "z", self.outputs_channel_index)
...
def compatible_shapes(ref_shape, target_shape):
    ...
    for ref_d, target_d in zip(reversed(ref_shape), reversed(target_shape)):
        if ref_d != target_d and target_d != 1:
            raise ValueError(
                f"{target_shape} is incompatible with {ref_shape}.")
    ...
    def _update_curvature_matrix_estimate(
        ...
        if self.has_scale:
            ...
            state.diagonal_factors[0].update(
                scale_diag_update, ema_old, ema_new)
        ...
        if self.has_shift:
            ...
            state.diagonal_factors[-1].update(shift_diag_update,
                                              ema_old, ema_new)
