...
def make_func_args(params, func_state, rng, batch, has_state: bool,
    ...
    if has_rng:
        if rng is None:
            raise ValueError(
                "The `rng` is None, but the argument `has_rng` is True.")
        func_args += (rng,)
    func_args += (batch,)
    return func_args
...
def inner_product(obj1: T, obj2: T) -> jnp.ndarray:
    ...
    elements_product = jax.tree_multimap(
...
class Stateful:
    ...
    def get_state(self) -> Mapping[str, Any]:
        ...
        for name in self.__stateful_fields_names:
            state[name] = Stateful._get_state_from_instance(
                getattr(self, name))
        return state
    ...
    def infer_class_state(class_type):
        ...
        fields_names = tuple(
            field.name for field in dataclasses.fields(class_type))
        original_init = getattr(class_type, "__init__", None)
        if original_init is None:

            def injected_init(self, *args, **kwargs):
                super(self.__class__, self).__init__(
                    *args, **kwargs)  # pylint: disable=bad-super-call
