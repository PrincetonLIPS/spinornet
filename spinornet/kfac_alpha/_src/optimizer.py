...
from spinornet.kfac_alpha._src import curvature_estimator
...
class Optimizer(utils.WithStagedMethods):
    ...
    def __init__(
        ...
        init_learn_rate: Optional[Numeric] = None,
        learn_rate_adjust: Optional[Numeric] = None,
        ...
        super().__init__(
            ...
            debug=debug,
            forbid_setting_attributes_after_finalize=forbid_setting_attributes_after_finalize,
        ...
        self._learn_rate_adjust = learn_rate_adjust
    ...
    @utils.staged
    def _setup_state_and_schedules2(
        self,
        learning_rate: Optional[Array],
        momentum: Optional[Array],
        damping: Optional[Array],
        ...
        learning_rate = jnp.where(
            minlrmod == 1, (1-self._learn_rate_adjust)*learning_rate, learning_rate)
        learning_rate = jnp.where(
            minlrmod > 1, (1+self._learn_rate_adjust)*learning_rate, learning_rate)
    ...
    def _compute_preconditioned_gradient(
        ...
        if self._norm_constraint is not None:
            ...
            max_coefficient = jnp.sqrt(
                self._norm_constraint / sq_norm_scaled_grads)
            coefficient = jnp.minimum(max_coefficient, 1)

            preconditioned_grads = utils.scalar_mul(
    ...
    def _compute_quad_change_for_damping(
        ...
        assert not (
            self._use_adaptive_learning_rate or self._use_adaptive_momentum)
    ...
    def _init(
        ...
        return Optimizer.State(
            ...
            # learning_rate=jnp.array(self._init_learn_rate, dtype=float),
            # minlrmod=jnp.array(1, dtype=float)
        )
    ...
    def _step(
        ...
        # Setup arguments
        # learning_rate, momentum, damping = self._setup_state_and_schedules2(
        #     state.learning_rate, momentum,
        #     state.damping if self._use_adaptive_damping else damping,
        #     state.step_counter, state.minlrmod)
        ...
        if self._include_per_param_norms_in_stats:
            param_norm_per_param = utils.per_parameter_norm(
                params, "param_norm")
        ...
        # Todo update damping by choosing larger than smallest neg eigv.
        # eigvals = self.estimator.eigenvalues(state.estimator_state, use_cached=False)
        # damping = max(1e-5, 1.05*jnp.abs(jnp.min(eigvals)))
        ...
        if self._include_per_param_norms_in_stats:
            update_norm_per_param = utils.per_parameter_norm(
                delta, "update_norm")
        ...
        stats = dict(
            ...
            eigenvalues=self.estimator.eigenvalues(
                state.estimator_state, use_cached=False),
        )
    ...
    def _solve_quad_model(
        ...
        A, A_damped, b = utils.pmean_if_pmap(
            (A, A_damped, b), self.pmap_axis_name)
        ...
        if all(c is None for c in fixed_coefficients):
            ...
            if len(fixed_coefficients) == 1:
                ...
                w = - lax.cond(special_case, lambda: b,
                               lambda: b / A_damped[0])
