...
def eval_jaxpr_eqn(eqn: JaxprEqn, in_values: Vars) -> Var:
    ...
    if jax_version > (0, 4, 11):
        user_context = jax.extend.source_info_util.user_context
    else:
        user_context = jax.core.source_info_util.user_context  # pytype: disable=module-attr
    with user_context(
            # with jax.extend.source_info_util.user_context(
...
def broadcast_in_dim_equivalent(
    ...
    if not (equation1.primitive.name == "broadcast_in_dim" and
        ...
        raise ValueError(
            "This is only applicable to `broadcast_in_dim` primitive.")
    return True
...
class GraphMatcherComparator:
    ...
    def __init__(
        ...
        self._special_eqn_equivalence_rules = dict(
            **special_eqn_equivalence_rules)
    ...
    def add_commutative_op_name(self, name: str):
        ...
        if name in self.commutative_ops_names:
            raise ValueError(
                f"Commutative op {name!r} has already been added.")
...
class JaxprGraph:
    ...
    var_to_creation_op: immutabledict.immutabledict = None
...
def make_jax_graph(
    ...
    closed_jaxpr, out_shapes = jax.make_jaxpr(
        func, return_shape=True)(*func_args)
    ...
    if clean_broadcasts:
        closed_jaxpr: ClosedJaxpr = merge_broadcasts_jaxpr(
            closed_jaxpr)  # pytype:disable=annotation-type-mismatch
...
class GraphMatch:
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, "param_graph_variables",
                           param_graph_variables)
    ...
    def create_eqn(
        ...
        in_vars = [self.variables_map[k]
                   for k in self.pattern.graph.jaxpr.invars]
...
def match_equations(
    ...
    for i, eqn in enumerate(reversed_eqns_to_match):
        ...
        if graph_eqn is None:
            assert first_output_var in (
                graph.jaxpr.invars + graph.jaxpr.constvars)
...
def clean_jaxpr(jaxpr: J, preserve_tags: bool = True) -> J:
    ...
    if dependants:
        raise ValueError(
            "Something went wrong with the dead code elimination.")
...
def _make_scale_and_shift_pattern(
    ...
    return GraphPattern(
        ...
        parameters_extractor_func=lambda jaxpr: dict(
...
def _auto_register_tags(
    ...
    for eqn in graph.jaxpr.eqns:
        ...
        if eqn_name == "cond":
            ...
            [eqn_params["body_jaxpr"]
             ] = final_jaxprs  # pylint:disable=unbalanced-tuple-unpacking
        elif eqn_name == "scan":
            [eqn_params["jaxpr"]] = final_jaxprs  # pylint:disable=unbalanced-tuple-unpacking
        elif eqn_name in ("xla_call", "xla_pmap"):
            [eqn_params["call_jaxpr"]
             ] = final_jaxprs  # pylint:disable=unbalanced-tuple-unpacking
        else:
            raise NotImplementedError()
    ...
    for eqn in mid_graph.jaxpr.eqns:
        ...
        for var in eqn.outvars:
            ...
            if match is not None:
                ...
                n = pattern_counters.get(tag_name, 0)
                pattern_counters[tag_name] = n + 1
                tag_locations.append(TagLocation(
                    eqns[-1], f"Auto[{tag_name}_{n}]"))
