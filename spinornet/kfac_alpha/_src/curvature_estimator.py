...
from spinornet.kfac_alpha._src import loss_functions
from spinornet.kfac_alpha._src import tracer
...
class ImplicitExactCurvature:
    ...
    def multiply_ggn_factor_transpose(
        ...
        losses, jacobian_vectors = self._loss_tags_jvp(
            func_args, parameter_structured_vector)
        vectors = self._multiply_loss_ggn_factor_transpose(
            losses, jacobian_vectors)
        batch_size = self.batch_size(func_args)
    ...
    def get_loss_inner_vector_shapes_and_batch_size(
        ...
        """
        losses, _ = self._loss_tags_vjp(
            func_args)  # pytype: disable=attribute-error  # always-use-return-annotations
        batch_size = self.batch_size(func_args)
...
class BlockDiagonalCurvature(
    ...
    def params_block_index(self) -> utils.Params:
        ...
        Returns:
          ...
          parameter.
        """
        params_block_index: list[Optional[int]] = [
            None] * self.num_params_variables
    ...
    def params_vector_to_blocks_vectors(
        ...
        params_values_flat = jax.tree_util.tree_leaves(
            parameter_structured_vector)
        blocks_vectors: list[Tuple[Array, ...]] = []

        for indices in self.jaxpr.layer_indices:
            blocks_vectors.append(
                tuple(params_values_flat[i] for i in indices))
    ...
    def multiply_matpower(
        ...
        identity_weight = utils.to_tuple_or_repeat(
            identity_weight, self.num_blocks)
        ...
        parameter_structured_result = self.blocks_vectors_to_params_vector(
            result)
    ...
    def eigenvalues(
        ...
        return jnp.concatenate([beig.flatten() for beig in blocks_eigenvalues], axis=0)
    ...
    def update_curvature_matrix_estimate(
        ...
        if estimation_mode == "fisher_gradients":
            ...
            keys = jax.random.split(rng, len(losses)) if len(
                losses) > 1 else [rng]
            vjp_vec = tuple(
            ...
            for loss, key in zip(losses, keys):
                ...
                if estimation_mode == "fisher_curvature_prop":
                    ...
                    vjp_vec.append(loss.multiply_fisher_factor(
                        random_b * 2.0 - 1.0))

                else:
                    shape = loss.ggn_factor_inner_shape
                    random_b = jax.random.bernoulli(key, shape=shape)
                    vjp_vec.append(loss.multiply_ggn_factor(
                        random_b * 2.0 - 1.0))
            ...
            for i, (loss, (loss_num_indices,)) in enumerate(zip(losses, shapes)):
                for index in range(loss_num_indices):
                    ...
                    if estimation_mode == "fisher_exact":
                        vjp_vec[i] = loss.multiply_fisher_factor_replicated_one_hot([
                                                                                    index])
                    else:
                        vjp_vec[i] = loss.multiply_ggn_factor_replicated_one_hot([
                                                                                 index])
                    ...
                    state = update_blocks(
                        tuple(vjp_vec), state, ema_old, ema_new)
    ...
    def update_cache(
        ...
        if self._distributed_cache_updates and pmap_axis_name is not None:
            ...
            new_states = utils.distribute_thunks(
                filtered_thunks, pmap_axis_name)
...
class ExplicitExactCurvature(BlockDiagonalCurvature):
    ...
    def _compute_losses_vjp(self, func_args):
        ...
        losses, losses_vjp = self._vjp(func_args)
