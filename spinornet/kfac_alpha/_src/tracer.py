        ...
        *args: Any,
        ...
        if jaxpr is None:
            ...
            if not allow_no_losses and not jaxpr.loss_tags:
                raise ValueError(
                    "No registered losses have been found during tracing.")
...
def _loss_tags_hvp(
    ...
    def df_dot_dv(p): return (jax.jvp(losses_sum, [p], [params_tangents])[1])
...
#    breakpoint()
        if iscomplex:
            tangents = jax.tree_map(lambda x: x+0j, tangents)
        ...
        tangents_dict.update(
            zip(processed_jaxpr.jaxpr.invars, inputs_tangents))
...
def layer_tags_vjp(
    ...
    **auto_registration_kwargs,
    ...
    return cached_transformation(
        func=func,
        transformation=functools.partial(_layer_tag_vjp, iscomplex=iscomplex),
        params_index=params_index,
        auto_register_tags=auto_register_tags,
