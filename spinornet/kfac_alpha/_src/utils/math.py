...
from spinornet.kfac_alpha._src.utils import types
...
def inner_product(
    ...
    if not types.abstract_objects_equal(obj1, obj2, check_dtype=False):
        raise ValueError(
            "The objects do not have identical abstract structure.")
...
def block_permuted(
    ...
    blocks = [jnp.split(row, indices, 1)
              for row in jnp.split(matrix, indices, 0)]
    reordered_blocks = [[blocks[i][j] for j in block_order]
...
def psd_inv_cholesky(matrix: Array, damping: Array) -> Array:
    ...
    if matrix.shape[:1] != matrix.shape[1:]:
        raise ValueError(
            f"Expected square matrix, but got shape {matrix.shape}.")
...
def psd_matrix_norm(
    ...
    if norm_type == "2_norm":
        ...
        if matrix.ndim == 0:
            ...
            raise ValueError(
                f"Unsupported shape for factor array: {matrix.shape}")
    ...
    *arrays: Array,
    ...
    def regular_inverse() -> Tuple[Array, ...]:
        ...
        for u in us:
            ...
            if u.size == 1:
                # damping not used in the scalar factors
                inv = jnp.ones_like(u)
...
def kronecker_product_axis_mul_v(
    ...
    for group, factor, f_str in zip(axis_groups, factors, factor_strs):
        ...
        vector_str = general_str[:min(group)] + \
            "y" + general_str[max(group) + 1:]
        ...
        # for "shared weights of e3nn" (specifically e3 Linear layers), but IDK if this is the right thing to do.
        try:
            result = r_next.reshape(v.shape)
        except:
            result = r_next.flatten()[0:v.size].reshape(v.shape)
...
def loop_and_parallelize_average(
    ...
    def average_func(*args) -> ArrayTree:
        ...
        _, output_tree = jax.make_jaxpr(
            func, return_shape=True)(*singleton_args)

        singleton_size = sum(
            x.size for x in jax.tree_util.tree_leaves(output_tree))
        ...
        remainder_args = jax.tree_util.tree_map(
            lambda x: x[all_chunks_size:], args)
