...
def main(_):
  ...
  for i, wftype in itertools.product(range(4),
        ...
        if wftype == 'coll':
            cfg.network.hf_method = 'HFOrbitalHEG2dcol'
            cfg.mcmc.spin_sample = False
        elif wftype == 'noncoll':
            cfg.network.hf_method = 'HFOrbitalHEG2d'
        ...
        cfg.log.save_path = f'heg2d_10_lb0_spinord_{wftype}_{units[i]}_{units[i]}_2l'

        cfg.optim.lr.rate = 0.1
        cfg.optim.lr.decay = 1
        cfg.optim.lr.delay = 10000
        cfg.optim.kfac.damping = 0.001
        cfg.optim.spring.damping = 0.001

        #cfg.log.restore_path = f'3_data_code/2deg-0lb/{wftype}-{units[i]}/'
